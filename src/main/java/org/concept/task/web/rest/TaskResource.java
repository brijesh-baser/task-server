...
import org.concept.task.service.UserService;
...
/**
 * REST controller for managing {@link org.concept.task.domain.Task}.
 */
@RestController
@RequestMapping("/api")
public class TaskResource {
     ...
     * @return the result of the search.
     */
    @GetMapping("/_all/tasks")
    public ResponseEntity<List<Task>> getAllTasks(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Tasks for query {}", query);
        Page<Task> page = taskService.getAllTask(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
     ...
     * @return the result of the search.
     */
    @GetMapping("/_pending/tasks")
    public ResponseEntity<List<Task>> getPendingTasks(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Tasks for query {}", query);
        Page<Task> page = taskService.getPendingTask(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
     ...
     * @return the result of the search.
     */
    @GetMapping("/_completed/tasks")
    public ResponseEntity<List<Task>> getCompletedTasks(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Tasks for query {}", query);
        Page<Task> page = taskService.getCompletedTask(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
